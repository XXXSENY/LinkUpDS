...
class LinkUpDB:
    ...
    def delete_post(self, post_id: str) -> bool:
        """Supprimer un post et ses relations."""
        query = """
    ...
    def get_feed(self, user_id: str, limit: int = 20, offset: int = 0):
        """Récupérer le feed d'un utilisateur (posts des utilisateurs suivis)."""
        query = """
        MATCH (u:User {userId: $user_id})-[:FOLLOWS]->(followed:User)-[:POSTED]->(p:Post)
        OPTIONAL MATCH (p)<-[:LIKES]-(liker:User)
        WITH p, followed, count(liker) AS likeCount
    ...
    def _execute_read(self, query: str, **params):
        ...
        with self._session() as s:
            return s.execute_read(work) if hasattr(s, "execute_read") else s.read_transaction(work)
    ...
    def _execute_write(self, query: str, **params):
        ...
        with self._session() as s:
            return s.execute_write(work) if hasattr(s, "execute_write") else s.write_transaction(work)
